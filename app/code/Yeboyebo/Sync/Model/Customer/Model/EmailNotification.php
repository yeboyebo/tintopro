...
namespace Yeboyebo\Sync\Model\Customer\Model;
...
class EmailNotification extends \Magento\Customer\Model\EmailNotification
{
	...
	public function newAccount(
		...
		$customerEmailData = $this->getFullCustomerObject($customer);
		$password = 'A'.substr(md5($customerEmailData->getData('taxvat')),0,7);
		$this->sendEmailTemplate(
			$customer,
			$types[$type],
			self::XML_PATH_REGISTER_EMAIL_IDENTITY,
			['customer' => $customerEmailData, 'back_url' => $backUrl, 'store' => $store, 'password' => $password],
			$storeId
		);
	}
}
